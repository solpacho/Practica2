{1.- Descargar el programa ImperativoEjercicioClase2.pas que contiene parte del código del siguiente enunciado a resolver. Implementar lo
indicado en el código.
Implementar un programa que invoque a los siguientes módulos.
a. Un módulo recursivo que retorne un vector de a lo sumo 15 números enteros “random” mayores a 10 y menores a 155 (incluidos ambos). La
carga finaliza con el valor 20.
...
f. Un módulo recursivo que reciba el vector generado en a) y un valor y devuelva verdadero si dicho valor se encuentra en el vector o falso
en caso contrario.
g. Un módulo que reciba el vector generado en a) e imprima, para cada número contenido en el vector, sus dígitos en el orden en que aparecen
en el número. Debe implementarse un módulo recursivo que reciba el número e imprima lo pedido. Ejemplo si se lee el valor 142, se debe 
imprimir 1 4 2.}
Program Clase2MI;
const dimF = 15;
      min = 10;
...
procedure ImprimirVector (v: vector; dimL: integer); // demasiados for
var
   i: integer;
begin
...
procedure ImprimirVectorRecursivo (v: vector; dimL: integer); // chequear
begin
	i:=1;
...
function  ObtenerMaximo (v: vector; dimL,max: integer): integer;//max deberia ser pasado por parametro como var (es funcion, deberia pasar a proc)
var
	max,mayor: integer;
begin
	if (dimL >=1) then begin
		...
		max:=ObtenerMaximo(v,dimL-1,max); // pasar max -1 en el principal
	end;
end; 

function  BuscarValor (v: vector; dimL, valor: integer): boolean; // preguntar

begin
	if (valor > max) or (valor < min) then
		BuscarValor:=false;
	else if v[dimL]=valor and dimL < dimF then
		BuscarValor:=true;
	else begin
