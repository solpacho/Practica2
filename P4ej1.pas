{Descargar el programa ImperativoEjercicioClase4.pas. y completar los módulos indicados en el código.
a. Almacenar los productos vendidos en una estructura eficiente para la búsqueda por código de producto. De cada producto deben quedar 
almacenados su código, la cantidad total de unidades vendidas y el monto total. De cada venta se cargan código de venta, código del producto
vendido, cantidad de unidades vendidas y precio unitario. El ingreso de las ventas finaliza cuando se lee el código de venta 0.
...
{ Retornar la cantidad de códigos que existen en el árbol que son menores que un valor que se recibe como parámetro }
  ...
  function CantidadDeCodigosMenores (a: arbol; cod: integer): integer; // preguntar
  {var
	cant: integer;
  begin
	cant:=0;
  ...
  begin
    ...
    if a <> nil then begin
      if a^.dato.codigo < cod then begin
        cant := cant + 1;
        cant := cant + CantidadDeCodigosMenores(a^.HI, cod);
        cant := cant + CantidadDeCodigosMenores(a^.HD, cod);
      end 
      else
        cant := cant + CantidadDeCodigosMenores(a^.HI, cod);
    end;
...
{Contenga un módulo que reciba la estructura generada en el punto a y dos códigos de producto y retorne el monto total entre todos los 
códigos de productos comprendidos entre los dos valores recibidos (sin incluir). }
  
  function ObtenerMontoTotalEntreDosCodigos (a: arbol; codigo1, codigo2: integer): real; // preguntar
  begin
    if a = nil then
		ObtenerMontoTotalEntreDosCodigos:=0
	else begin
		if (a^.dato.codigo < codigo2) and (a^.dato.codigo > 1) then 
...
begin
  ...
  writeln ('----- Modulo E ----->');
  writeln;
  write ('Ingrese el primer codigo de producto: ');
  readln (codigo1);
