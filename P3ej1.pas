...
numero de socio, nombre y edad. La carga finaliza con el numero de socio 0 y el arbol debe quedar ordenado por numero de socio. La 
informacion de cada socio debe generarse aleatoriamente.
b. Una vez generado el arbol, realice modulos independientes que reciban el arbol como parametro para: 
    ...
    3. Informar el número de socio con mayor edad. Debe invocar a un módulo recursivo que retorne dicho valor.
    4. Aumentar en 1 la edad de los socios con edad impar e informar la cantidad de socios que se les aumento la edad.
    5. Leer un nombre e informar si existe o no existe un socio con ese nombre. Debe invocar a un módulo recursivo que reciba el nombre 
leído y retorne verdadero o falso.
    6. Informar la cantidad de socios. Debe invocar a un módulo recursivo que retorne dicha cantidad.
    7.Informar el promedio de edad de los socios. Debe invocar a un módulo recursivo que retorne el promedio de las edades de los socios.
}
...
procedure InformarSociosOrdenDereciente (a: arbol);
{ Informar los datos de los socios en orden creciente. }
...
{ Informar el numero de socio con mayor edad. Debe invocar a un modulo recursivo que retorne dicho valor.  }
	...
	procedure NumeroMasEdad (a: arbol; var maxEdad: integer; var maxNum: integer); // pasa al siguiente con los llamados
	begin
	   if (a <> nil) then
	   begin
  ...
  begin
     if (a = nil) then
		AumentarEdad:= 0
            ...
            AumentarEdad:= resto + AumentarEdad (a^.HI) + AumentarEdad (a^.HD); // llama y manda los valores por parámetro
          end;  
  end;
...
begin
	if (a = nil) then 
		...
		if (a^.dato.nombre = n) then
			...
			existe:= InformarExistenciaNombreSocio(a^.hi,n);
			if (not existe) then 
				existe:=InformarExistenciaNombreSocio(a^.hd,n);
			InformarExistenciaNombreSocio:=existe;
		end;
end;
procedure calcular(a:arbol);
...
var
	suma: integer; cantidad: integer; prom: real;
begin
	InformarCantidadSocios(a); InformarPromedioDeEdad(a);
	suma:= InformarPromedioDeEdad(a);
	cantidad:= InformarCantidadSocios(a);
	prom:=suma/cantidad;
	writeln('la cantidad de socios es de ',InformarCantidadSocios(a));
  writeln;
...
Begin
  ...
  InformarSociosOrdenDereciente (a); // hecho
  InformarNumeroSocioConMasEdad (a);
  {AumentarEdadNumeroImpar (a);}
  readln(n);
  InformarExistenciaNombreSocio (a,n); 
  writeln(InformarExistenciaNombreSocio(a,n));
  calcular(a);
